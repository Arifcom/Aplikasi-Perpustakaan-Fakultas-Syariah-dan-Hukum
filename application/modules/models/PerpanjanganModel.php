...
class PerpanjanganModel extends BaseModel {
    ...
    public static function Monitor() {
        $query = self::db()->prepare("SELECT * FROM transaksi WHERE transaksi = 'Perpanjangan'");
        $query->execute();
        $result = array();
        while ($row = $query->fetch()) {
            array_push($result, new PerpanjanganModel($row["nim"], $row["kode_buku_1"], $row["kode_buku_2"], $row["kode_buku_3"], $row["tanggal"], $row["waktu"]));
        }
        return $result;
    }
    ...
    public static function tambah() {
        $nim = $_SESSION['username'];
        $query = self::db()->prepare("UPDATE transaksi SET transaksi = 'Perpanjangan' WHERE nim = '$nim'");
        $query->execute();
    }
    ...
    public static function export() {
        ...
        header('Content-Disposition: attachment;filename="Laporan Perpanjangan Buku Perpustakaan.xls"');
